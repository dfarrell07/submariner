...
function verify_subm_engine_container() {
  ...
  kubectl exec -it $subm_engine_pod_name --namespace=$subm_ns -- env | grep "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
  if [[ $operator = true ]]; then
    kubectl exec -it $subm_engine_pod_name --namespace=$subm_ns -- env | grep "HOSTNAME=$subm_engine_pod_name"
  else
...
function verify_subm_routeagent_container() {
  ...
  for subm_routeagent_pod_name in "${subm_routeagent_pod_names_array[@]}"; do
    ...
    if [[ $operator = true ]]; then
      kubectl exec -it $subm_routeagent_pod_name --namespace=$subm_ns -- env | grep "HOSTNAME=$subm_routeagent_pod_name"
    else
